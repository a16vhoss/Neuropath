...
BEGIN
    ...
    WITH student_stats AS (
        ...
        WHERE 
            -- Include if they have any progress OR are enrolled in the class (if set belongs to class)
    ...
    SELECT 
        ...
        -- Ranking Score: (Mastery % * 0.6) + (Quiz % * 0.4)
        -- If no cards, rely fully on quiz. If no quiz, rely fully on cards.
        CASE 
            WHEN v_total_cards > 0 AND ss.total_quizzes > 0 THEN
                ((ss.mastered_count::numeric / v_total_cards::numeric) * 100 * 0.6) + (ss.avg_score * 0.4)
            WHEN v_total_cards > 0 THEN
                ((ss.mastered_count::numeric / v_total_cards::numeric) * 100)
